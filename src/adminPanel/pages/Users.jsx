...
import UserPlanModal from "../modals/UserPlanModal";
import { MdCardMembership } from "react-icons/md";

export default function Users() {
  const [users, setUsers] = useState([]);
  const [isAddUserModal, setIsAddUserModal] = useState(false);
  const [isDocsModal, setIsDocsModal] = useState(false);
  ...
  const handleAddUser = (item) => {
    handleAddItem(item, "users/create", setUsers);
  };

  const handleEditUser = (item) => {
    handleEditItem(item, "users/update/" + selectedItem?._id, setUsers);
  };

  const handleDeleteUser = () => {
    handleDeleteItem("users/delete/" + selectedItem?._id, setUsers);
  };
  const handleDocs = (item, url) => {
    uploadUserDocs(item, url, setUsers);
  };
  ...
  return (
    <>
      {isAddUserModal && (
        <UserModal
          saveDocs={handleDocs}
          saveFunc={handleAddUser}
          setIsModal={setIsAddUserModal}
        />
      )}
      {isEditUserModal && (
        <UserModal
          saveFunc={handleEditUser}
          saveDocs={handleDocs}
          setIsModal={setIsEditUserModal}
          view="edit"
      ...
      {isDeleteUserModal && (
        <ConfirmModal
          ...
          setIsModal={setIsDeleteUserModal}
          heading={`Delete user "${selectedItem.fName} ${selectedItem.lName}" details?`}
        />
      )}
      <AdminLayout>
        <div>
          <h2 className="md:text-[32px] text-[24px] text-gray-500">Users</h2>
        </div>
        {isLoading ? (
          <LoadingMutatingDots />
        ) : (
          <div className="shadow-md shadow-gray-400 text-gray-600 rounded p-4 my-5 text-[13px] bg-white">
            ...
            {users?.length > 0 ? (
              ...
              <p className="text-center">Add users to see here</p>
            )}
          </div>
        )}
      </AdminLayout>
