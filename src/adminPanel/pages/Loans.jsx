...
export default function Loans() {
  ...
  const handleAddLoan = (item) => {
    handleAddItem(item, 'loan/create', setLoans);
  };

  const handleEditLoan = (item) => {
    // console.log(item,"UPDATED")
    handleEditItem(item, 'loan/update/' + selectedItem._id, setLoans);
  };

  const handleDeleteLoan = () => {
    handleDeleteItem('loan/delete/' + selectedItem._id, setLoans);
  };

  useEffect(() => {
    fetchAllData('loan/get', setLoans, setIsLoading);
  }, []);

  return (
    <>
      {isAddLoanModal && (
        <LoanModal saveFunc={handleAddLoan} setIsModal={setIsAddLoanModal} />
      )}
      {isEditLoanModal && (
        <LoanModal
      ...
      {isDeleteLoanModal && (
        <ConfirmModal
          ...
          heading={`Delete Loan details from ${selectedItem.bankName}?`}
        />
      )}
      <AdminLayout>
        <div>
          <h2 className="md:text-[32px] text-[24px] text-gray-500">Loans</h2>
        </div>
        {isLoading ? (
          <LoadingMutatingDots />
        ) : (
                        ...
                        return (
                          ...
                          >
                            {row.cells.map((cell, i) => {
                              ...
                              else if (cell.column.id.includes('collatral'))
                                return (
                                  <td
                                    {...cell.getCellProps()}
                                    key={i}
                                  ...
                                  >
                                    {cell.value.eligible
                                      ? `About ${cell.value.minAmount} Lakhs`
                                      : 'Not Eligible'}
                                  </td>
                                );
                              else
...
const COLUMNS = [
  ...
  {
    Header: 'With Collateral',
    accessor: 'withCollateral',
    id: 'collatral1',
  },
  {
    Header: 'Without Collateral',
    accessor: 'withoutCollateral',
    id: 'collatral2',
  },
  {
    Header: 'Actions',
