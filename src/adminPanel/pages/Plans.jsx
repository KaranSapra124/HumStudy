...
export default function Plans() {
  ...
  const [isViewPlanModal, setIsViewPlanModal] = useState(false);
  const [isDeletePlanModal, setIsDeletePlanModal] = useState(false);
  ...
  const handleAddPayment = (item) => {
    handleAddItem(item, "plans/create", setPlans);
  };

  const handleEditPayment = (item) => {
    handleEditItem(item, "plans/update/" + selectedItem?._id, setPlans);
  };

  const handleDeletePayment = () => {
    handleDeleteItem("plans/delete/" + selectedItem?._id, setPlans);
  };

  useEffect(() => {
    fetchAllData("plans/get", setPlans, setIsLoading);
  }, []);

  return (
    <>
      {isAddPlanModal && (
        <PlanModal saveFunc={handleAddPayment} setIsModal={setIsAddPlanModal} />
      )}
      {isEditPlanModal && (
        <PlanModal
          saveFunc={handleEditPayment}
          setIsModal={setIsEditPlanModal}
          view="edit"
      ...
      {isDeletePlanModal && (
        <ConfirmModal
          ...
          setIsModal={setIsDeletePlanModal}
          heading={`Delete Payment "${selectedItem.paymentId}" details?`}
        />
      )}
      <AdminLayout>
        <div>
          <h2 className="md:text-[32px] text-[24px] text-gray-500">Plans</h2>
        </div>
        {isLoading ? (
          <LoadingMutatingDots />
        ) : (
          <div className="shadow-md shadow-gray-400 text-gray-600 rounded p-4 my-5 text-[13px] bg-white">
            ...
            {plans?.length > 0 ? (
              ...
              <p className="text-center">Add Plans detail to see here</p>
            )}
          </div>
        )}
      </AdminLayout>
